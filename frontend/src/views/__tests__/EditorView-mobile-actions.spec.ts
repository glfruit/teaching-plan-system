...
describe('EditorView mobile quick actions', () => {
  ...
  it('provides local draft center dialog with history restore controls', () => {
    ...
    expect(editorViewSource).toContain('快速骨架模板')
    expect(editorViewSource).toContain('推荐：')
    expect(editorViewSource).toContain('使用推荐')
    expect(editorViewSource).toContain('导出前预检')
    expect(editorViewSource).toContain('仅填空')
    expect(editorViewSource).toContain('覆盖套用')
    expect(editorViewSource).toContain('定位到')
    expect(editorViewSource).toContain('一键修复')
    expect(editorViewSource).toContain('全部修复')
  ...
  it('binds keyboard shortcuts for save, export and publish actions', () => {
    ...
    expect(editorViewSource).toContain('for (const action of SHORTCUT_ACTIONS)')
    expect(editorViewSource).toContain('matchesShortcut(event, shortcutConfig.value[action.id])')
    expect(editorViewSource).toContain('await runShortcutAction(action.id)')
    expect(editorViewSource).toContain('handleOpenShortcutDialog()')
    expect(editorViewSource).toContain('handleSaveShortcutConfig')
  })
})
